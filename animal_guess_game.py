...
Show me your first animal! 🎉"""
        ...
        if self.ai_assistant:
            ...
            # Interactive countdown instead of silent sleep
            self._perform_countdown_and_capture(user)
    ...
    def _perform_countdown_and_capture(self, user: str) -> None:
        """Perform an interactive countdown before capturing."""
        user_name = user.title()
        
        if self.ai_assistant:
            # Give initial instruction
            self.ai_assistant.speak(f"Get ready {user_name}! I'm going to count to three, then take the picture!", user)
            time.sleep(1)
            
            # Count down
            self.ai_assistant.speak("Three!", user)
            time.sleep(1)
            
            self.ai_assistant.speak("Two!", user)
            time.sleep(1)
            
            self.ai_assistant.speak("One!", user)
            time.sleep(1)
            
            self.ai_assistant.speak("Cheese! Taking the picture now!", user)
        else:
            # Fallback for no audio
            print("Get ready! Counting down...")
            for i in [3, 2, 1]:
                print(f"{i}...")
                time.sleep(1)
            print("Capturing now!")
